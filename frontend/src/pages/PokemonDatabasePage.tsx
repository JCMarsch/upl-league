...
function getSpriteSize(mode: SpriteMode): string {
  return mode === '3d' || mode === '3d-shiny' ? 'w-12 h-12' : 'w-10 h-10'
}
...
export default function PokemonDatabasePage() {
  ...
  const [spriteMode, setSpriteMode] = useState<SpriteMode>(() => {
    return (localStorage.getItem('spriteMode') as SpriteMode) || '2d'
  })
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-gray-100 text-left">
                ...
                <SortHeader col="type1" label="Type(s)" />
            ...
            <tbody>
              {filtered.map((p) => {
                ...
                const imgSize = getSpriteSize(spriteMode)
                return (
                  <tr key={p.id} className="border-t hover:bg-gray-50">
                    <td className="px-3 py-1">
                      {spriteUrl ? (
                        <img src={spriteUrl} alt={p.species_name} className={imgSize} style={{ objectFit: 'contain' }} />
                      ) : (
                        <div className={`${imgSize} bg-gray-200 rounded`} />
                      )}
                    </td>
