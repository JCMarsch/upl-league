...
interface WishlistEntry {
  ...
  conditions: { type: 'already_have' | 'pokemon_gone'; species_id: number }[]
  species_name: string | null
  species_sprite_url: string | null
  tier: string | null
  point_cost: number | null
...
function WishlistSection({ teamId, seasonId }: { teamId: number; seasonId: number }) {
  ...
  const [allPokemon, setAllPokemon] = useState<AvailPokemon[]>([])
  ...
  const loadWishlist = () =>
    axios.get(`/teams/${teamId}/wishlist`, { withCredentials: true }).then(r => setWishlist(r.data))

  useEffect(() => {
    loadWishlist()
    axios.get(`/seasons/${seasonId}/pokemon`).then(r => setAllPokemon(r.data.filter((p: AvailPokemon) => p.is_available !== false)))
  }, [teamId, seasonId])

  const addItem = async (sp: AvailPokemon) => {
    await axios.post(`/teams/${teamId}/wishlist`, { season_pokemon_id: sp.id }, { withCredentials: true })
    setSearch(''); setShowSearch(false)
    loadWishlist()
  }

  const removeItem = async (id: number) => {
    await axios.delete(`/teams/${teamId}/wishlist/${id}`, { withCredentials: true })
    loadWishlist()
  }

  const reorder = async (newList: WishlistEntry[]) => {
    setWishlist(newList)
    await axios.post(`/teams/${teamId}/wishlist/reorder`, { ordered_ids: newList.map(w => w.id) }, { withCredentials: true })
  }

  const updateConditions = async (item: WishlistEntry, operator: string | null, conditions: WishlistEntry['conditions']) => {
    await axios.patch(`/teams/${teamId}/wishlist/${item.id}`,
      { season_pokemon_id: item.season_pokemon_id, conditions_operator: operator, conditions },
  ...
  const moveDown = (idx: number) => {
    if (idx === wishlist.length - 1) return
    const next = [...wishlist]
  ...
  const onDragEnd = () => { reorder(wishlist); setDragIdx(null) }

  const searchResults = allPokemon.filter(p => {
    const name = p.species_name?.toLowerCase() ?? ''
    const q = search.toLowerCase()
    return q && name.includes(q) && !wishlist.find(w => w.season_pokemon_id === p.id)
  }).slice(0, 8)

  const inpStyle = {
    border: '1px solid var(--color-border)', borderRadius: 6, padding: '5px 8px',
  ...
  return (
    <div className="rounded-xl border p-4 space-y-3" style={{ borderColor: 'var(--color-border)', background: 'var(--color-surface)' }}>
      ...
      <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
        Pokemon are auto-picked from this list (in priority order) if your timer expires.
      </p>
      ...
      {showSearch && (
        <div className="relative">
          ...
          {searchResults.length > 0 && (
            <div className="absolute z-20 w-full rounded-lg border shadow-lg mt-1" style={{ borderColor: 'var(--color-border)', background: 'var(--color-surface)' }}>
              {searchResults.map(p => (
                <button
                  key={p.id}
                  onClick={() => addItem(p)}
                  className="w-full flex items-center gap-2 px-3 py-2 text-left hover:opacity-80 border-b last:border-0 text-sm"
                  style={{ borderColor: 'var(--color-border)' }}
                >
                  {p.species_sprite_url && <img src={p.species_sprite_url} alt="" className="w-7 h-7 object-contain" />}
                  <span className="flex-1">{p.species_name}</span>
                  <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                    {p.tier} · {p.point_cost ?? '?'}pt
                  </span>
                  {p.drafted_by_team_id && <span className="text-xs text-red-400">drafted</span>}
                </button>
              ))}
            </div>
          )}
      ...
      {wishlist.length === 0 ? (
        <div className="text-xs py-3 text-center" style={{ color: 'var(--color-text-muted)' }}>
          No wishlist items yet. Add pokemon above.
        </div>
      ) : (
          ...
          {wishlist.map((item, idx) => (
            <div key={item.id}>
              <div
                ...
                onDragEnd={onDragEnd}
                className="flex items-center gap-2 rounded-lg px-2 py-1.5 cursor-grab active:cursor-grabbing"
                style={{
                  background: dragIdx === idx ? 'var(--color-bg)' : undefined,
                  border: '1px solid var(--color-border)',
                  opacity: item.is_available ? 1 : 0.45,
                  userSelect: 'none',
                }}
              >
                <span className="text-xs font-mono w-4 text-center" style={{ color: 'var(--color-text-muted)' }}>{idx + 1}</span>
                {item.species_sprite_url && <img src={item.species_sprite_url} alt="" className="w-8 h-8 object-contain" />}
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate">{item.species_name}</div>
                  <div className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                    {item.tier} · {item.point_cost ?? '?'}pt
                    {!item.is_available && <span className="ml-1 text-red-400">drafted</span>}
                  </div>
                </div>
                <div className="flex gap-1 items-center">
                  <button onClick={() => setExpandedId(expandedId === item.id ? null : item.id)} className="text-xs px-1.5 py-0.5 rounded" style={{ background: 'var(--color-bg)', color: 'var(--color-text-muted)', border: '1px solid var(--color-border)' }} title="Add conditions">if…</button>
                  <button onClick={() => moveUp(idx)} disabled={idx === 0} className="text-xs px-1.5 disabled:opacity-30" style={{ color: 'var(--color-text-muted)' }}>↑</button>
                  <button onClick={() => moveDown(idx)} disabled={idx === wishlist.length - 1} className="text-xs px-1.5 disabled:opacity-30" style={{ color: 'var(--color-text-muted)' }}>↓</button>
                  <button onClick={() => removeItem(item.id)} className="text-xs px-1.5" style={{ color: '#ef4444' }}>✕</button>
                </div>
              </div>
              ...
              {expandedId === item.id && (
                <ConditionsEditor
                  ...
                  onSave={(op, conds) => { updateConditions(item, op, conds); setExpandedId(null) }}
...
function ConditionsEditor({
  ...
  item: WishlistEntry
  allPokemon: AvailPokemon[]
  onSave: (op: string | null, conds: WishlistEntry['conditions']) => void
  onCancel: () => void
}) {
  const [operator, setOperator] = useState<string>(item.conditions_operator ?? 'AND')
  const [conditions, setConditions] = useState<WishlistEntry['conditions']>(item.conditions ?? [])
  const [search, setSearch] = useState('')

  const addCond = (type: 'already_have' | 'pokemon_gone', species_id: number) => {
    setConditions(prev => [...prev, { type, species_id }])
    setSearch('')
  }
  ...
  const searchResults = allPokemon.filter(p => {
    const q = search.toLowerCase()
    return q && (p.species_name?.toLowerCase().includes(q)) && !conditions.find(c => c.species_id === p.id)
  }).slice(0, 6)

  const speciesName = (id: number) => allPokemon.find(p => p.id === id)?.species_name ?? `#${id}`

  const inpStyle = { border: '1px solid var(--color-border)', borderRadius: 4, padding: '4px 7px', background: 'var(--color-bg)', color: 'var(--color-text)', fontSize: '0.75rem' }

  return (
    <div className="mt-1 mb-2 rounded-lg border p-3 space-y-2" style={{ borderColor: 'var(--color-border)', background: 'var(--color-bg)' }}>
      <div className="text-xs font-medium" style={{ color: 'var(--color-text-muted)' }}>
        Auto-pick <strong>{item.species_name}</strong> only if…
      </div>

      {conditions.length > 1 && (
        <div className="flex gap-2 items-center">
          <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Match:</span>
          {(['AND', 'OR'] as const).map(op => (
            <button key={op} onClick={() => setOperator(op)}
              className="text-xs px-2 py-0.5 rounded"
              style={{ background: operator === op ? 'var(--color-primary)' : 'var(--color-surface)', color: operator === op ? 'white' : 'inherit', border: '1px solid var(--color-border)' }}>
      ...
      {conditions.map((c, i) => (
        <div key={i} className="flex items-center gap-2 text-xs">
          <span style={{ color: 'var(--color-text-muted)' }}>{c.type === 'already_have' ? 'I already have' : 'Gone (drafted)'}:</span>
          <span className="font-medium">{speciesName(c.species_id)}</span>
          <button onClick={() => removeCond(i)} style={{ color: '#ef4444' }}>✕</button>
        </div>
      ))}

      <div className="relative">
        <input placeholder="Search pokemon…" value={search} onChange={e => setSearch(e.target.value)} style={{ ...inpStyle, width: '100%' }} />
        {searchResults.length > 0 && (
          <div className="absolute z-30 w-full rounded border shadow-lg mt-0.5" style={{ borderColor: 'var(--color-border)', background: 'var(--color-surface)' }}>
            {searchResults.map(p => (
              <div key={p.id} className="px-2 py-1 text-xs" style={{ borderBottom: '1px solid var(--color-border)' }}>
                <span className="font-medium mr-2">{p.species_name}</span>
                <button onClick={() => addCond('already_have', p.id)} className="mr-2 underline" style={{ color: 'var(--color-primary)' }}>I have it</button>
                <button onClick={() => addCond('pokemon_gone', p.id)} className="underline" style={{ color: '#f59e0b' }}>It's gone</button>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="flex gap-2 pt-1">
        <button onClick={() => onSave(conditions.length > 0 ? operator : null, conditions)}
          className="text-xs px-3 py-1 rounded text-white" style={{ background: 'var(--color-primary)' }}>Save</button>
        <button onClick={onCancel} className="text-xs px-3 py-1 rounded border" style={{ borderColor: 'var(--color-border)' }}>Cancel</button>
        {conditions.length > 0 && (
          <button onClick={() => onSave(null, [])} className="text-xs px-3 py-1 rounded border ml-auto" style={{ borderColor: '#ef4444', color: '#ef4444' }}>Clear conditions</button>
        )}
      </div>
    </div>
  )
...
export default function TeamPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Wishlist — only visible to manager/admin */}
      {canEdit && (
        <WishlistSection teamId={team.id} seasonId={team.season_id} />
      )}
